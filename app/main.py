...
from pydantic import BaseModel # Pydantic-malli JSON bodylle
...
client = OpenAI(
    api_key=os.getenv("OPENROUTER_API_KEY"), # Haetaan API-avain ympäristömuuttujista
    base_url="https://openrouter.ai/api/v1", # OpenRouterin API-osoite
)
...
class AskRequest(BaseModel): # Pydantic-malli /ask-endpointin JSON bodylle
    file_id: str # Tiedoston uniikki tunniste
...
async def ask(request: AskRequest): # Otetaan vastaan JSON body AskRequest-mallin mukaisesti
    matches = list(UPLOAD_DIR.glob(f"{request.file_id}_*")) # Etsitään tiedosto uploads-kansiosta
    ...
    text = text[:8000] # Rajataan teksti, ettei quota/tokenit pauku

    try:
        response = client.chat.completions.create( # Kysytään OpenRouterilta vastaus
            model="stepfun/step-3.5-flash:free", # Ilmainen malli
            messages=[
                {"role": "system", "content": "Vastaa käyttäjän kysymykseen dokumentin perusteella."}, # System prompt
                {"role": "user", "content": f"DOKUMENTTI:\n{text}\n\nKYSYMYS:\n{request.question}"}, # Dokumentti + kysymys
            ],
        )
        ...
        raise HTTPException(status_code=502, detail=str(e)) # Selkeä virhe HTTP 502 -bad gateway, jos OpenRouter ei vastaa tai tapahtuu muu virhe